...
SELECT f.title as MusicTitle, i.store_id as MusicStore,
ROW_NUMBER() OVER (PARTITION BY i.film_id, i.store_id ) AS MusicCopyNo,
CASE WHEN '2005-08-01%' BETWEEN rental_date AND return_date THEN c.email ELSE 'Not Rented' END AS MusisCustEmail
FROM inventory i
INNER JOIN film f
...
GROUP BY i.inventory_id, f.film_id, i.store_id;
