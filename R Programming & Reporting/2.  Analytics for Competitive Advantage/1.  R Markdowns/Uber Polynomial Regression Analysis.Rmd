---
title: "Uber Polynomial Regression Analysis"
...
## as time increases, Uber increases. As time increases, vehicles sales change 
...
reg4 <- plm(lnSales ~ Uber + as.factor(monthid), index = c("city_id", "monthid"), 
            model = "within", data = vs)
summary(reg4)

reg5 <- plm(lnSales ~ Uber + as.factor(monthid) + lndidi + lnyidao + lnshenzhou + 
              ln_highwaypassengertraffic + ln_aviationpassengertraffic	+ 
              ln_grpcity	+ 
              ln_grppercapitacity +	ln_mobilesubscriberscity +	
              ln_internetsubscriberscity +	ln_roadarea
            + ln_buses	+ ln_buspassengervolume	+ ln_taxis	+ 
              ln_busesper1000	+ ln_roadareapercapita
            + ln_populationcity	+ ln_populationdensitycity	+ ln_employedcity	+ 
              ln_unemployedcity	+ ln_wageofstaffandworkercity, 
